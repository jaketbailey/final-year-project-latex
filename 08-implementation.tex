...
The initial stages of development began using a first draft of elicited requirements \see{appendix:initial-user-requirements} where later on, a new set of user requirements was formed based on primary research \see{litrev:competingproducts}. This method proved effective in not delaying initial development, where the core requirements were unlikely to change greatly. Furthermore, Iteration 1 (i1) consisted of implementing the building blocks of the artefact, including setting up map \see{iteration1:leaflet-osm} and core routing functionalities \see{iteration1:basic-routing}.
...
Unlike i1 \see{implementation:iteration1}, Iteration 2 (i2) development was conducted after the primary research \see{litrev:competingproducts} and requirements review \see{chap:requirements} had been completed. The remaining updated requirements were prioritised and distributed between i2 and Iteration 3 (i3) \see{implementation:iteration3}. I2 builds on i1, enhancing existing functionality and introducing new features. 
...
Finally, i3 included the remainder of the critical and desired requirements. These included round-trip routing \see{iteration3:round-trip}, route import \see{iteration3:route-import}, Garmin Connect integration \see{iteration3:garmin-integration} and social media sharing \see{iteration3:social-sharing}. There were only a few requirements that were not implemented due to time constraints, these were the lowest priority requirements and were not critical to the artefact's functionality.

\subsection{Round Trip Routing}
\label{iteration3:round-trip}

The ORS API was implemented shortly after it was found that OSRM didn't support round-trip routing, only A to B routing \see{iteration1:basic-routing}. The new API was implemented towards the end of i1 \see{implementation:iteration1} ahead of knowing the requirements for i3. 
...
The development of the artefact proved challenging, but highly rewarding. Most functionality was implemented with time to spare, illustrating the effective task and time management strategies outlined in \see{chap:pm}. In the following chapter, the testing strategies are covered \see{chap:testing}, demonstrating how crucial regular testing is. The testing methods used ensured that implementation delivered an artefact at a high standard, meeting most requirements and all deadlines.